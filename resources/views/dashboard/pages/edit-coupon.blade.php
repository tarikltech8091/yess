	...
	<div class="row margin_top_20">
		<div class="col-sm-12">
			...
			<div class="panel panel-default">
				...
				<form method="post" action="{{url('/dashboard/coupon/update-'.$edit_coupon_info->coupon_id)}}" enctype="multipart/form-data">
				...
				<div class="col-sm-6">
					...
					<div class="form-group">
						...
						<select class="form-control select_category" name="coupon_category_id">
							...
							<option {{($edit_coupon_info->coupon_category_id == $list->category_id) ? "selected" :''}} value="{{$list->category_id}}">{{$list->category_name}}</option>
							@endforeach
							@endif
						</select>
					</div>
					<div class="form-group">
						...
						<select class="form-control sub_category_list" name="coupon_sub_category_id">
							<option value="{{$edit_coupon_info->coupon_sub_category_id}}">{{$edit_coupon_info->sub_category_name}}</option>
					...
					<div class="form-group">
						...
						<select class="form-control select_merchant_list" name="coupon_merchant_id">
							...
							<option {{($edit_coupon_info->coupon_merchant_id == $list->merchant_id) ? "selected" :''}} value="{{$list->merchant_id}}">{{$list->merchant_name}}</option>
					...
					<div class="form-group">
						...
						<select class="form-control branch_list" name="coupon_branch_id">
							<option value="{{$edit_coupon_info->coupon_branch_id}}">{{$edit_coupon_info->branch_name}}</option>
						</select>
					</div>

					<div class="form-group">
						<label> Coupon Keyword <span class="symbol required"></span></label>
						<input type="text" id="form-field-3" class="form-control" name="coupon_keyword" value="{{$edit_coupon_info->coupon_keyword}}">
					</div>
				...
				<div class="col-sm-6">
					...
					<div class="form-group">
						...
						<div class="input-group">
			                <input type="text" data-date-format="yyyy-mm-dd" data-date-viewmode="years" class="form-control date-picker" name="coupon_opening_date" value="{{$edit_coupon_info->coupon_opening_date}}" placeholder="">
